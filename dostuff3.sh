...
curl https://raw.githubusercontent.com/minhealthnz/nz-covid-data/main/cases/covid-case-counts.csv -o covid-cases-counts.csv
    #   curl https://raw.githubusercontent.com/minhealthnz/nz-covid-data/f081fae524ef7625ac1d96e633d68bf3aacc4a0a/cases/weekly-deaths.csv -o deaths.csv
#curl https://enf.tracing.covid19.govt.nz/api/pages/stats -o btstats.json
#       curl https://raw.githubusercontent.com/minhealthnz/nz-covid-data/f081fae524ef7625ac1d96e633d68bf3aacc4a0a/cases/weekly-hospitalisations-for-covid.csv -o hospital.csv
...
reinfections() {
    ...
    echo "Welly reinfections:"
    cat reinfection.csv  | grep $TODAY | grep -f wellydhb.txt | awk -F "," '{ sum+=$8;} END{print sum;}'
    echo "chch/wc reinfections:"
    cat reinfection.csv  | grep $TODAY | grep -f chchdhb.txt | awk -F "," '{ sum+=$8;} END{print sum;}'
}
nz() {
...
deaths() {
        echo "doing latest deaths (mondays)"
        tail -n 1 deaths.csv | awk -F "," '{ print $3 }'
        echo "deaths done"

}

hospitalisations() {
        echo "doing hospitalisations"
        LATEST=$(cat hospital.csv | grep Age |  sort | tail -n 1 | awk -F "," '{ print $1 }')
        echo $LATEST
        cat hospital.csv | grep Age | sort | grep $LATEST | awk -F "," '{ sum+=$3;} END{print sum;}'
}
wellington
chch
auckland
